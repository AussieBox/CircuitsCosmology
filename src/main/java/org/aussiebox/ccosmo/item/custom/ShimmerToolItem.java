...
public class ShimmerToolItem extends MiningToolItem {
    ...
    public boolean canMine(BlockState state, World world, BlockPos pos, PlayerEntity miner) {
        if (miner.getMainHandStack().get(ModDataComponentTypes.SHIMMER_TOOL_TYPE) == CCOSMOConstants.ShimmerToolType.TRIDENT && !miner.isInCreativeMode()) return false;
        return true;
    }

    @Override
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (CCOSMOUtil.stackHasEnchantment(world, stack, CCOSMOConstants.BORDERLINKED_ENCHANT))
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (stack.get(ModDataComponentTypes.SHIMMER_TOOL_TYPE) == CCOSMOConstants.ShimmerToolType.PICKAXE) {
            if (!user.isSneaking()) {
                if (stack.getOrDefault(ModDataComponentTypes.SHIMMER_TOOL_CHARGES, 0) <= 0) return TypedActionResult.fail(stack);
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (stack.get(ModDataComponentTypes.SHIMMER_TOOL_TYPE) == CCOSMOConstants.ShimmerToolType.AXE) {
            if (user != null && !user.isSneaking()) {
                ...
                if (shouldCancelStripAttempt(context)) {
                    ...
                    if (optional.isEmpty()) {
                        ...
                        boolean hasBorderlinked = CCOSMOUtil.stackHasEnchantment(world, stack, CCOSMOConstants.BORDERLINKED_ENCHANT);
    ...
    public boolean isUsedOnRelease(ItemStack stack) {
        return stack.get(ModDataComponentTypes.SHIMMER_TOOL_TYPE) == CCOSMOConstants.ShimmerToolType.TRIDENT;
    }
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (stack.get(ModDataComponentTypes.SHIMMER_TOOL_TYPE) != CCOSMOConstants.ShimmerToolType.TRIDENT) return;
    ...
    public boolean onClicked(ItemStack stack, ItemStack otherStack, Slot slot, ClickType clickType, PlayerEntity player, StackReference cursorStackReference) {
        if (clickType == ClickType.RIGHT) {
            if (stack.getOrDefault(ModDataComponentTypes.SHIMMER_TOOL_CHARGES, 0) < stack.getOrDefault(ModDataComponentTypes.SHIMMER_TOOL_MAX_CHARGES, 5) && otherStack.isOf(ModItems.SHIMMER_POWDER)) {
                ...
                List<CCOSMOConstants.ShimmerToolSkin> skinList = Arrays.stream(CCOSMOConstants.ShimmerToolSkin.values()).toList();
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> list, TooltipType type) {
        ...
        list.add(1, Text.translatable("item_skin.ccosmo.prefix")
                .withColor(0xFFFFFF)
                .append(Text.translatable("item_skin.ccosmo.shimmer_tool." + stack.getOrDefault(ModDataComponentTypes.SHIMMER_TOOL_SKIN, CCOSMOConstants.ShimmerToolSkin.BASE).asString())
                        .withColor(0xFF55FF)));
        list.add(2, bar);
        list.add(3, Text.literal(" "));
        if (charges == 0) list.add(2, Text.translatable("tip.ccosmo.shimmer_tool_refill").withColor(0xA024FF));
    }

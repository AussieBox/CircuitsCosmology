...
public class ShimmerforkEntity extends PersistentProjectileEntity {
    ...
    public ShimmerforkEntity(World world, LivingEntity owner, ItemStack stack) {
        ...
        this.dataTracker.set(BLOCK_CHANGES_REMAINING, BOBConstants.shimmerforkBlockChangeMaximum);
        ...
        if (!this.dealtDamage) {
            ...
            if (entityList != null && !entityList.isEmpty()) {
                ...
                if (entity != null && entity != this.getOwner()) {
                    DamageSource damageSource = this.getDamageSources().create(BOBConstants.SHIMMERFORK_DAMAGE, this.getOwner());
    ...
    public void onLand() {
        ...
        World world = this.getWorld();
        double range = Arrays.stream(BOBConstants.shimmerforkLandEffectData).toList().getFirst();
        double blocksAffected = Arrays.stream(BOBConstants.shimmerforkLandEffectData).toList().getLast();

...
public class ShimmerforkEntityRenderer extends EntityRenderer<ShimmerforkEntity> {
    ...
    private final ShimmerforkEntityModel model;

    public ShimmerforkEntityRenderer(EntityRendererFactory.Context context) {
        super(context);
        this.model = new ShimmerforkEntityModel(context.getPart(EntityModelLayers.TRIDENT));
    }

    public void render(ShimmerforkEntity tridentEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        matrixStack.push();
    ...
    @Override
    public Identifier getTexture(ShimmerforkEntity tridentEntity) {
        return TEXTURE;
    }
}
